...
def main() -> int:
    ...
    app.setApplicationName(config.application_name); app.setOrganizationName(config.company_name)
    app.setStyleSheet(DARK_STYLE if preferences.get_bool("dark_mode") else LIGHT_STYLE)
    if QFontDatabase.addApplicationFont(str(resource_path("assets/DejaVuSans.ttf"))) < 0:
        logging.warning("Bundled print font could not be loaded")
    ...
    def handle_exception(exc_type, exc_value, exc_tb) -> None:
        ...
        QMessageBox.critical(None, "Unexpected Error", "CheqMint encountered an unexpected error. Details were saved to the log.")
